...
func CreateProjectEnvironmentHandler(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	params, err := mcputils.ParamsParser[CreateProjectEnvironmentParams](args)
	if err != nil {
		return nil, fmt.Errorf("failed to parse parameters: %w", err)
	}

	return makePostRequest(fmt.Sprintf("/v1/projects/%s/environments", params.ProjectID), params.Body, authInfo)
}
