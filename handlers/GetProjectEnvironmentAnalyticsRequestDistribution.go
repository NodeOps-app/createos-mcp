...
type GetProjectEnvironmentAnalyticsRequestDistributionParams struct {
...
func GetProjectEnvironmentAnalyticsRequestDistributionHandler(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	params, err := mcputils.ParamsParser[GetProjectEnvironmentAnalyticsRequestDistributionParams](args)
	if err != nil {
		return nil, err
	}
	...
	return makeGetRequest(fmt.Sprintf("/v1/projects/%s/environments/%s/analytics/request-distribution", params.ProjectID, params.EnvironmentID), queryParams, apiKey)
}
